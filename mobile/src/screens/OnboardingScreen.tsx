...
export function OnboardingScreen() {
  ...
  return <Screen>
    ...
    {step === 6 && <><ShieldCheck size={38} color={colors.success} /><Title subtitle="Your setup is ready. Choose how you’d like to record the first transaction later.">Ready for clearer finances</Title><Card><View style={{ flexDirection: 'row', gap: 12 }}><MessageSquareText size={22} color={colors.primary} /><Text style={{ color: colors.text, flex: 1 }}>Import transaction text from the Add flow.</Text></View></Card><Card><View style={{ flexDirection: 'row', gap: 12 }}><Landmark size={22} color={colors.primary} /><Text style={{ color: colors.text, flex: 1 }}>Enter an expense, income, or transfer record manually.</Text></View></Card></>}
