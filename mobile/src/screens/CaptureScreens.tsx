...
export function AddMenuScreen({ navigation }: any) {
  ...
  const actions = [
    ...
    ['Record transfer', 'TransferForm', ArrowLeftRight, colors.primary], ['Import transaction text', 'MessageImport', MessageSquareText, colors.warning],
...
export function ExpenseFormScreen({ navigation, route }: any) {
  ...
  return <Screen>
    ...
    <Card><Text style={{ color: colors.text, fontWeight: '900' }}>Payment source</Text>{([['current_account', 'Account'], ['savings', 'Savings account'], ['credit_card', 'Credit card'], ['cash', 'Cash'], ['other', 'Other']] as const).map(([value, label]) => <Choice key={value} selected={form.paymentMethod === value} label={label} onPress={() => payment(value)} />)}
    ...
    {form.paymentMethod !== 'credit_card' && <Card><Text style={{ color: colors.text, fontWeight: '900' }}>Count in month</Text><Text style={{ color: colors.textMuted }}>Select which month's salary this expense should reduce.</Text>{monthNames.map((label, index) => <Choice key={label} selected={Number(form.planningMonth) === index + 1} label={label} onPress={() => setForm({ ...form, planningMonth: index + 1 })} />)}<Field label="Year" value={String(form.planningYear)} onChangeText={(planningYear) => setForm({ ...form, planningYear: Number(planningYear) })} keyboardType="number-pad" /></Card>}
...
export function IncomeFormScreen({ navigation, route }: any) {
  ...
  const incomeDate = preset.date ? new Date(preset.date) : new Date(); const salaryFundingDate = new Date(incomeDate.getFullYear(), incomeDate.getMonth() + 1, 1); const presetIsSalary = /^salary$/i.test(String(preset.source || ''));
  const [form, setForm] = useState<any>({ amount: preset.amount ? String(preset.amount) : '', date: preset.date ? String(preset.date).slice(0, 10) : today(), memberId: refId(preset.memberId), source: preset.source || 'Salary', description: preset.description || '', savingsAccountId: refId(preset.savingsAccountId), fundingMonth: preset.fundingMonth || (presetIsSalary ? salaryFundingDate.getMonth() + 1 : preset.month) || salaryFundingDate.getMonth() + 1, fundingYear: preset.fundingYear || (presetIsSalary ? salaryFundingDate.getFullYear() : preset.year) || salaryFundingDate.getFullYear(), imported: !!preset.imported, expectedUpdatedAt: preset.updatedAt });
  useEffect(() => { if (options && !form.memberId) setForm((value: any) => ({ ...value, memberId: idOf(options.members[0] || {} as Option) })); }, [options]);
  ...
  return <Screen><Title subtitle="Record money received and where it landed.">Add income</Title><AmountField value={form.amount} onChange={(amount) => setForm({ ...form, amount })} currency={user?.currency || 'AED'} />
    ...
    <Card><Text>Destination</Text><Choice selected={!form.savingsAccountId} label="Member account" onPress={() => setForm({ ...form, savingsAccountId: '' })} />{options!.accounts.filter((a) => a.type === 'savings').map((item) => <Choice key={item.key} selected={form.savingsAccountId === item.id} label={item.name} onPress={() => setForm({ ...form, savingsAccountId: item.id })} />)}</Card>
    <Card><Text style={{ fontWeight: '900' }}>Funds which month?</Text><Text>Salary automatically funds the following month. You can override it.</Text>{monthNames.map((label, index) => <Choice key={label} selected={Number(form.fundingMonth) === index + 1} label={label} onPress={() => setForm({ ...form, fundingMonth: index + 1, fundingOverride: true })} />)}<Field label="Funding year" value={String(form.fundingYear)} onChangeText={(fundingYear) => setForm({ ...form, fundingYear: Number(fundingYear), fundingOverride: true })} keyboardType="number-pad" /></Card>
...
export function TransferFormScreen({ navigation, route }: any) {
  ...
  const [form, setForm] = useState<any>({ amount: preset.amount ? String(preset.amount) : '', date: preset.date ? String(preset.date).slice(0, 10) : today(), from: preset.from || accountKey('from'), to: preset.to || accountKey('to'), description: preset.description || '', notes: preset.notes || '', budgetTreatment: preset.budgetTreatment || 'internal', planningMonth: preset.planningMonth || preset.month || new Date().getMonth() + 1, planningYear: preset.planningYear || preset.year || new Date().getFullYear(), expectedUpdatedAt: preset.expectedUpdatedAt || preset.updatedAt });
  useEffect(() => {
  ...
  return <Screen><Title subtitle="Card payments are recorded as transfers to avoid duplicate expense records.">Record transfer</Title><AmountField value={form.amount} onChange={(amount) => setForm({ ...form, amount })} currency={user?.currency || 'AED'} />
    ...
    {to?.type !== 'credit_card' && <Card><Text style={{ fontWeight: '900' }}>Monthly-plan treatment</Text>{([['internal', 'Internal transfer'], ['monthly_expense', 'Count as an expense'], ['savings_sweep', 'Move surplus to savings'], ['savings_draw', 'Borrow from savings']] as const).map(([value, label]) => <Choice key={value} selected={form.budgetTreatment === value} label={label} onPress={() => setForm({ ...form, budgetTreatment: value })} />)}{form.budgetTreatment === 'monthly_expense' && <><Text>Expense month</Text>{monthNames.map((label, index) => <Choice key={label} selected={Number(form.planningMonth) === index + 1} label={label} onPress={() => setForm({ ...form, planningMonth: index + 1 })} />)}<Field label="Year" value={String(form.planningYear)} onChangeText={(planningYear) => setForm({ ...form, planningYear: Number(planningYear) })} keyboardType="number-pad" /></>}</Card>}
...
export function RecoveryFormScreen({ navigation, route }: any) {
  ...
  return <Screen><Title subtitle="Recoveries reduce net spending without changing the original gross expense.">Expense recovery</Title><Card><Text>Gross: {expense.amount}</Text><Text>Recovered: {expense.recoverySummary.recoveredAmount}</Text><Text>Net remaining: {remaining}</Text></Card>
    ...
    <AmountField value={form.amount} onChange={(amount) => setForm({ ...form, amount })} currency="" /><Field label="Date" value={form.date} onChangeText={(date) => setForm({ ...form, date })} /><Card><Text>Source</Text>{([['bank_reimbursement', 'Payment reimbursement'], ['family_transfer', 'Family transfer'], ['employer', 'Employer'], ['friend', 'Friend'], ['other', 'Other']] as const).map(([value, label]) => <Choice key={value} selected={form.source === value} label={label} onPress={() => setForm({ ...form, source: value })} />)}</Card><Field label="Reason or notes" value={form.notes} onChangeText={(notes) => setForm({ ...form, notes })} /><Button label={online ? 'Save recovery' : 'Queue recovery'} onPress={save} /></Screen>;
}
...
export function MessageImportScreen({ navigation }: any) {
  ...
  async function analyze() { setBusy(true); try { setResult((await api.importMessage(message)).data); } catch (e) { Alert.alert('Could not extract transaction', errorMessage(e)); } finally { setBusy(false); } }
  function review() {
