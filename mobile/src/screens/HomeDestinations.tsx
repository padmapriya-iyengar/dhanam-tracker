...
import { moneyTone, useAppTheme } from '../theme';
...
export function ActivityDetailScreen({ route, navigation }: any) {
  ...
  return <Screen><Title subtitle={`${item.type} · ${new Date(item.date).toLocaleString()}`}>{item.title}</Title><Card><Text style={{ color: colors.textMuted }}>Amount</Text><Text style={{ color: moneyTone(colors, item.type === 'expense' ? 'expense' : item.type === 'transfer' ? 'transfer' : item.type === 'recovery' ? 'recovery' : 'income'), fontSize: 28, fontWeight: '900' }}>{prefs.privacyMode ? '••••••' : money(item.type === 'transfer' ? item.transferAmount || 0 : item.amount)}</Text><Text style={{ color: colors.textMuted }}>Account</Text><Text style={{ color: colors.text, fontWeight: '800' }}>{item.account}</Text><Text style={{ color: colors.textMuted }}>Member</Text><Text style={{ color: colors.text }}>{item.member || 'Household'}</Text>{record?.recoverySummary && <><Text style={{ color: colors.textMuted }}>Gross / recovered / net</Text><Text style={{ color: colors.text }}>{money(record.amount)} / {money(record.recoverySummary.recoveredAmount)} / {money(record.recoverySummary.netAmount)}</Text></>}</Card>
