    ...
    <div className="space-y-5">
      ...
      <Modal isOpen={detailModal === 'cards'} onClose={() => setDetailModal(null)} title={`Closed card statements · ${selectedLabel}`} size="lg">
        <div className="space-y-3">
          {!funding.cardDues?.length && <p className="rounded-lg bg-slate-50 p-4 text-sm text-slate-500">No closed card statements have a recorded balance.</p>}
          {(funding.cardDues || []).map((card) => (
            <div key={`${card.creditCardId}-${card.cycleEnd}`} className="rounded-xl border border-slate-100 p-4">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-sm">
                <div><p className="text-xs text-slate-400">Debits</p><p className="mt-1 font-semibold text-slate-700"><Money value={card.amount} /></p></div>
                <div><p className="text-xs text-slate-400">Credits</p><p className="mt-1 font-semibold text-emerald-700"><Money value={card.paid} /></p></div>
